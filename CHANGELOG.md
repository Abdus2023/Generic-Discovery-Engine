...
## [1.6.0] — 2026-09-13 — Bounded Knowledge Kernel (B1→B3 retention coherence)

- **Retention (B2+B3):** `CONFIG.retention` unified policy `visited{2k} candidateKeys{2k} candidateHistory{2k} observations{800+5M/150} discoveries{2k} resources{2k×100} fingerprint{500×100} patterns{500} clusters{500} networkEvents{1k} graphEdges{5k} diagnostics{500}` + `runtimeBudget.maxBodyBytes 5M` (mirrors `maxBodiesInMemory 150`, was declared but not enforced → `800×2M=1.6B` possible). `ResourceRecord.merge` now `.slice(-100)` after `unique` for `types/mechanisms/parents/candidateIds/observationIds/discoveryIds/networkEventIds` (was `unique` unbounded → `2000×∞`). `KnowledgeBase` now **Bounded Knowledge Kernel** 1019 lines: `_enforceVisitedBound` FIFO `visited-evicted` + `_enforceCandidateKeysBound` historical-first (preserves live frontier, `candidateKeys-evicted`/`pressure`) + `_enforceCandidateHistoryBound` evicts oldest `completed/skipped` FIFO + cleans `candidateKeys`/`claimed`/`resource.candidateIds` (`candidate-history-evicted`) + `_removeObservationReferences`/`_removeDiscoveryReferences`/`_removeCandidateReferences`/`_removeResourceFingerprint` for referential coherence (was `Map.delete` only → dangling `discoveryIds→[D17]` after eviction) + stale `candidateKeys→missing` cleaned (`candidateKeys-stale-cleaned`) + `recordObservation` now `_enforceFingerprintBound` (500 hashes FIFO, 100 urls/hash `fingerprint-url-evicted`) + `_enforceBodyBudget` FIFO clears oldest bodies when `>150 bodies||>5M bytes` (`body-evicted`, was count only) + `addDiscovery`/`ensureResource` FIFO with cleanup (`discovery-evicted`/`resource-evicted` now cleans refs) + `addEdge` FIFO shift `graphEdge-evicted` (was drop) + `recordPattern` → `_enforcePatternBound` (`pattern-evicted`/`cluster-evicted`). `serialize()` now `visited.slice(-2k)` (was unbounded `[...visited]`). `restore()` now bounded restoration: enforces `candidateHistory/candidateKeys/visited/observations/discoveries/resources/fingerprint/pattern/edges/diagnostics` after loading (was direct insert → `5000 persisted` could bypass `2000 runtime`), diagnostics `*-evicted-on-restore`. B1 containers → B2 payload → B3 coherent graph.
- **Security:** `GM_xmlhttpRequest.onload` now enforces redirect boundary: `finalUrl= response.finalUrl||plan.target`; if `sameOriginOnly && finalUrl!==plan.target && !isAllowedUrl(canonicalizeUrl(finalUrl))` → `ledger gm-redirect-blocked` + `Observation{status:blocked, reason:redirect-cross-origin}` blocked (was fetch-hardened `redirect:'error'` only, GM still allowed cross-origin redirect). `GenericDiscoveryEngine.recordNetworkEvent` now reads `retention.networkEvents.maxEntries ?? maxNetworkEvents` FIFO `network-events-evicted`.
- **Build:** `src/header.txt` `1.5.0→1.6.0` + 1.6 patch notes (retention+slice+kernel+GM), `package 1.6.0` `description 32 ADRs`. `scripts/verify-build.js` adds 8 gates: `CONFIG.retention` `maxBodyBytes` `maxRelationsPerResource` `body-evicted` `gm-redirect-blocked` `_enforceVisitedBound` `_removeObservationReferences` `observation-evicted-on-restore`. `dist` `6453→6794 (+341)` `508e3e…→7efbfd…` `223914B`, min `66k 32.7% bf049e…→74k 33.4% ba83e9…`, ESM `267B 792783` unchanged, providerRatio `20.6%→18.9%`. `src/knowledge.js` `744→1019`, `src/models.js` slice, `src/config.js` `+retention`, `src/engine.js` GM+network.
- **Tests:** existing **160/160 42 suites** still PASS; new stress proves B3 semantically (12/12: relation cap 100, body budget 5M/150 50 bodies, visited 2k, discovery/observation dangling cleaned, fingerprint 500×100, pattern 500, restore 2k/800/5k, GM blocked, stale cleaned, history 2k, edges 5k). `coverage:check` still honest `0/0/0`.
- **Docs:** `docs/adr/032-bounded-knowledge-kernel.md` + `adr/README 32` + `DECISIONS 32` + `architecture/OVERVIEW 1.6.0` + `src/README 1.6.0 7efbfd… 6794` + `VERIFICATION_SUPPLEMENT_v1.6.0` (planned, this changelog interim).

## [1.5.0] — 2026-09-13 — Verification Hardening + Runtime Bounds (P0/P1 audit fixes)

- **Verification (P0):** `.c8rc.json` honest `all:true` `include src/**/*.js` + `dist/generic-discovery-engine.user.js` `exclude tests/scripts/dist` `lines:0 branches:0 functions:0` (was `include tests` 85/75/80 misleading 99% of tests); `tsconfig.json` now `include src/**/*.js` + `dist`+`tests`; `package.json` `lint` honest `eslint . --max-warnings=50` + syntax check (was `node --check` only) + `typecheck` without `| head` masking (exit propagates, was `2>&1 | head -n 100 || echo` masked). `eslint` added `eslint@9` `globals` ` @eslint/js` + `eslint.config.js` now lints `src/**/*.js` with `browser+node` globals (`trustedTypes`/`PerformanceObserver`/etc.) + `dist/*.min.js/*.esm.js` ignored + `scripts/**/*.js` node globals + `no-undef:off` for generated. `dist` grew `6394→6453` (+59) `508e3e…` `204882B`, min `66k 32.7%` `bf049e…`, ESM `267B` `792783…`, providerRatio `21.1→20.6%`. `scripts/verify-build.js` now asserts P1 fixes: `redirect:error` when `sameOriginOnly`, `// // @connect` least-privilege, `maxDiscoveriesInMemory`/`maxResourcesInMemory`, `diagnosticCount`, `configuredProviders`, `adaptive-target`, `concurrencyTarget`.
- **Runtime (P1):** `CONFIG.maxDiscoveriesInMemory 2000` + `maxResourcesInMemory 2000` + `runtimeBudget {maxBodiesInMemory:150, maxDiscoveryHistory:2000, maxResourceHistory:2000}`; `KnowledgeBase.addDiscovery`/`ensureResource` now FIFO evict oldest when `size >= max` + diagnostic `discovery-evicted`/`resource-evicted` (was unbounded, only persistence slice). `GenericDiscoveryEngine.recordNetworkEvent` now FIFO evicts oldest (was drop) + `handleBridgeEvent` centralizes via `recordNetworkEvent` (cap bypass fixed). `fetch` now `redirect: CONFIG.sameOriginOnly ? 'error' : 'follow'` (was `follow`, redirect boundary). `src/header.txt` `@connect self` only, wildcard commented `// // @connect *` (lease-privilege).
- **Health+Concurrency (P1):** `getHealthMetrics` dead branch fixed `diagnosticCount = db.diagnostics.length` vs sliced `recentDiagnostics.length > max` (never true) → `diagnosticCount > maxErr` (now reachable); `providerHealth` now `totalProviders: configuredProviders (13)` + `instantiatedProviders` distinct (was `Object.keys(providerMetrics).length` conflated). `adaptive concurrency` clarified as target: `get concurrencyTarget/set`, workers fixed at scan start, diagnostics `adaptive-target-increase/decrease` + legacy `adaptive-increase` for compat, note `target for next scan`.
- **Build:** `src/header.txt` `1.4.0→1.5.0` + patch notes vs 1.4 (no `src tree` inside block comment to avoid `*/` close, was `src/**/*.js` broke `/*`). `package.json` `1.5.0` `honest coverage` + `eslint` devDeps. `npm test` still **160/160 42 suites**, `coverage:check` now honest `0/0/0` (production src not yet exercised, was 99% of tests), `lint` honest (eslint + syntax), `typecheck` unmasked (known DOM/GM errors informational).
- **Docs:** `docs/adr/031-verification-hardening-runtime-bounds.md` + `docs/adr/README.md` → 31 ADRs (30→31). `docs/DECISIONS.md`/`docs/architecture/OVERVIEW.md`/`src/README.md` bumped to `6,453` lines. `VERIFICATION_SUPPLEMENT_v1.5.0` planned (this changelog is interim).
