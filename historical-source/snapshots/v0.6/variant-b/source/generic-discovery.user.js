...
(function () {
     ...
     *       ↺
     *
     * This is a web-resource discovery engine.
     *
     * It is NOT:
     *   - RF scanning
     *   - DVB tuning
    ...
    function canonicalizeUrl(value, base = location.href) {
        ...
        /*
         * These are intentionally not treated as web-resource candidates.
         */
        if (
        ...
        try {
            ...
            if (CONFIG.stripTrackingParams) {
                ...
                for (const key of keys) {
                    ...
                    const prefixMatch =
                        CONFIG.trackingParameterPrefixes.some(
                            prefix =>
                                lower.startsWith(
                                    prefix.toLowerCase()
                                )
                    ...
                    const exactMatch =
                        CONFIG.trackingParameters.some(
                            parameter =>
                                lower ===
    ...
    function looksLikeHtml(body) {
        return /<!doctype\s+html|<html[\s>]|<head[\s>]|<body[\s>]/i
            .test(body || '');
    }
    ...
    function looksLikeXml(body) {
        return /<\?xml\b|<(?:urlset|sitemapindex|rss|feed|sitemap)\b/i
            .test(body || '');
    }
    ...
    function makeFingerprint(observation) {
        ...
        return {
            ...
            contentLength:
                observation.http?.contentLength ?? null,
    ...
    /*
     * Network instrumentation begins at document-start.
     *
     * DOM seeding waits until DOMContentLoaded so that initial resources,
     * links, forms, manifests, etc. can be observed.
     */
    const initialSeed =
        () => {
            try {
