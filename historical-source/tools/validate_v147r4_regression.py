#!/usr/bin/env python3
"""Isolated predecessor regression and append-only guard checks for v14.7-R4."""
import hashlib
import json
import os
import shutil
...
def main():
    ...
    rows = []
    with tempfile.TemporaryDirectory(prefix="gde-r4-reg-") as temporary:
        root = Path(temporary)
        ...
        cases = [
            ("REG-01", "validate_executable_boundary_contracts_v147r3.py", 0),
            ("REG-02", "validate_frozen_boundary_realization_v147r21.py", 0),
            ("REG-03", "validate_executable_realization_v147r2.py", 0),
            ("REG-04", "validate_v147_conformance_correction_v1471.py", 0),
            ("REG-05", "build_executable_boundary_contracts_v147r3.py", "REFUSE"),
            ("REG-06", "build_reference_domain_model_v147r4.py", "REFUSE"),
            ("REG-07", "build_frozen_boundary_realization_v147r21.py", "REFUSE"),
        ]
    ...
    preserved = prior["protected_artifact_count"] == len(prior["artifacts"]) == 961 and not mismatches
    rows.append({"check_id": "REG-08", "script": "PRIOR-INTEGRITY.yaml", "returncode": 0 if preserved else 1,
                 "expected": 0, "passed": preserved, "output_tail": f"mismatches={mismatches[:10]}"})
    ok = all(row["passed"] for row in rows)
    output = {"realization_version": "14.7-R4", "method": "ISOLATED_PREDECESSOR_VALIDATION_AND_SUCCESSOR_GUARDS",
