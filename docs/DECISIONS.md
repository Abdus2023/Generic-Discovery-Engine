...
## Decisions extracted (v1.1.0 — 24 ADRs)
...
3. Verification (v0.7.1 audit) files P0/P1; patches landed v0.7.2/v0.7.3/v0.7.4/v0.7.5/v0.7.6/v0.7.7/v0.7.8/v0.7.9/v0.8.0/v0.8.1/v0.8.2/v0.9.0/v0.9.1/v1.0.0/v1.1.0.

## Further splits
- The 2.2 MB file also contains 12 full code fences. The runnable artifact is now `dist/generic-discovery-engine.user.js` (v1.1.0, 6,076 lines, `6dcfa8…` + `dist/generic-discovery-engine.min.js` 58k 32.3%, built from `src/` 7 modules) (was v1.0.0 5,977 / v0.9.1 5,969 / v0.9.0 5,875). The fences are retained for diff archaeology but are no longer the source of truth.
- With 24 ADRs + rAF + coverage + TTL + gates + lifecycle + concurrency + pattern/cluster + build determinism + robots/headers + change detection + modular prelude + export hardening + framework bundler + pattern-guided/revisit + stable + lazy providers/esbuild the control-plane is **stable 1.1** for v1.1.0; `docs/analysis/DEEP_DVB_AUDIT_v0.8.2.md` remains valid (135/135) and `src/` is source of truth — 1.1 adds provider hardening with `build:all` minify.
