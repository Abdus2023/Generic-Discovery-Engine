...
```
src/ 7-file mirror (new in 0.8.1, ADR 019)
 ├── config.js — CONFIG v8 ~84 keys ES export (real extract)
...
dist/generic-discovery-engine.user.js (5,784 lines) still source of truth; src/ is verified mirror, not yet bundled (scripts/build.js checks src 7 exist before hashing).
```
...
- Narrow `@connect` shipped v0.7.4; Trusted Types `gde-bridge` + fuzz + priority invariants shipped v0.7.5/v0.7.6; TTL + FIFO + throttle + gates shipped v0.7.7; lifecycle + concurrency + typecheck shipped v0.7.8; pattern/cluster + build determinism shipped v0.7.9; robots/headers + change detection shipped v0.8.0; modular prelude + export hardening shipped v0.8.1.
- Planning doc fully split is incremental; this overview + 19 ADRs + coverage gates + TTL + lifecycle + concurrency + pattern/cluster + build determinism + robots/headers + change detection + modular prelude completes the v0.8.1 feature pass; `npm run coverage:check` 85/75/80 gate, `npm run coverage` 99% line, `npm run typecheck` informational, `npm run verify:build` deterministic (sha256 031c3… lines 5784, src 7).
