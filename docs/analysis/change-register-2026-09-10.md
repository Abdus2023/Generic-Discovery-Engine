...
| Authorization | one `state: GRANTED` object on the `PUSH` profile, whose 21 explicit capability grants (not the profile name) are the binding constraint: 10 effective, 11 `DENIED` | full contract, effective capability derivation and rollback in [scope-and-authorization.md](scope-and-authorization.md) §5; machine-readable in [analysis.json](analysis.json) and [authorization.yaml](authorization.yaml) |
