# v0.7.1 — Replayable Acquisition Decisions

> **Status:** CURRENT
>
> **Source:** `Continue Architecture Planning.md`
>
> **Purpose:** Complete v0.7.1 userscript: deterministic event ledger and replayable acquisition decisions.
...
- *Turn lead-in* — `CAP-043` — `Continue Architecture Planning.md` L48868–54113
...
<!-- CAP-043 | Continue Architecture Planning.md L48868–54113 | turn 27 -->
> **Source sections:** `CAP-043`

### v0.7.1 — Replayable Acquisition Decisions + Deterministic Event Ledger

The next step is to make the engine **auditable at the decision level**.
...
We cannot guarantee that a future execution of `GET /x` will return the same bytes.

We _can_ guarantee that, given the same candidate and policy/configuration, the engine can explain and reproduce the **decision path**:
...
> **Every executable acquisition must have an explicit `AcquisitionPlan`.**
...
This version is deliberately conservative. It does **not** redesign the providers again. It strengthens the control plane around the existing v0.6 architecture.
