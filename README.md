...
> **Current release:** `v0.7.7` — determinism & bounds (see [`VERIFICATION_REPORT.md`](VERIFICATION_REPORT.md), [`VERIFICATION_SUPPLEMENT_v0.7.2.md`](VERIFICATION_SUPPLEMENT_v0.7.2.md), [`VERIFICATION_SUPPLEMENT_v0.7.6.md`](VERIFICATION_SUPPLEMENT_v0.7.6.md), [`VERIFICATION_SUPPLEMENT_v0.7.7.md`](VERIFICATION_SUPPLEMENT_v0.7.7.md), [`CHANGELOG.md`](CHANGELOG.md), [`dist/generic-discovery-engine.user.js`](dist/generic-discovery-engine.user.js)). `node --check` PASS · `npm test` 73/73 PASS (incl. 6-test E2E + hardening + 7 fuzz + 8 priority + 13 determinism) · `v0.7.1` archived as `dist/generic-discovery-engine.v0.7.1.user.js` · new: [`docs/SECURITY_AUDIT.md`](docs/SECURITY_AUDIT.md), [`docs/PERFORMANCE_ANALYSIS.md`](docs/PERFORMANCE_ANALYSIS.md), [`docs/DECISIONS.md`](docs/DECISIONS.md), [`docs/architecture/OVERVIEW.md`](docs/architecture/OVERVIEW.md), [`docs/adr/`](docs/adr/) 9 ADRs, `tests/fuzz-extract.test.js` + `tests/property-*.test.js`, `.c8rc.json` (85/75/80 gate) + `npm run coverage:check` (99% line) · `eslint.config.js` + `.prettierrc` + `docs/ci/verify.yml.example` (lint+gate).
...
Current response providers (v0.7.7 — 7 providers, ordered pipeline):
...
## Architecture (v0.7.7 — TTL-bounded, coverage observable, privacy-opt-in)
...
> **v0.7.7 delta:** `CONFIG.candidateTTL` 0/off → `ttl-expired` sweep in `claimNextCandidate()` + Ledger FIFO 5k / Origin interval 150 ms / `fnv1a32` sampling invariants via `tests/property-determinism.test.js` (12 cases) + `.c8rc` gate 85/75/80 + 3 ADRs (007-009) + `VERIFICATION_SUPPLEMENT_v0.7.7.md`. **v0.7.6 delta:** rAF-batched `updateUI` (`_uiRaf` + `_doUpdateUI`) + `npm run coverage` 99% + 8 invariants. **v0.7.5 delta:** Trusted Types `gde-bridge` + JSDoc + `tests/fuzz-extract.test.js` + 3 ADRs + CI `docs/ci/verify.yml.example`. **v0.7.4 delta:** `CONFIG.privacy` scrub + `@connect self` + `csp-blocks-bridge` + ADR split 3 →6. **v0.7.3 delta:** `getCoverageMetrics()` frontier + `export.coverage`. See `VERIFICATION_REPORT.md` §2.2 and `CHANGELOG.md`.  
## Candidate Lifecycle
