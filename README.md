...
> **Current release:** `v0.7.5` — trust & verification (see [`VERIFICATION_REPORT.md`](VERIFICATION_REPORT.md), [`VERIFICATION_SUPPLEMENT_v0.7.2.md`](VERIFICATION_SUPPLEMENT_v0.7.2.md), [`CHANGELOG.md`](CHANGELOG.md), [`dist/generic-discovery-engine.user.js`](dist/generic-discovery-engine.user.js)). `node --check` PASS · `npm test` 50/50 PASS (incl. 6-test E2E + hardening + 7 fuzz) · `v0.7.1` archived as `dist/generic-discovery-engine.v0.7.1.user.js` · new: [`docs/SECURITY_AUDIT.md`](docs/SECURITY_AUDIT.md), [`docs/PERFORMANCE_ANALYSIS.md`](docs/PERFORMANCE_ANALYSIS.md), [`docs/DECISIONS.md`](docs/DECISIONS.md), [`docs/architecture/OVERVIEW.md`](docs/architecture/OVERVIEW.md), [`docs/adr/`](docs/adr/) 6 ADRs, `tests/fuzz-extract.test.js` · `eslint.config.js` + `.prettierrc` + `.github/workflows/verify.yml`.
...
Current response providers (v0.7.5 — 7 providers, ordered pipeline):
...
## Architecture (v0.7.5 — trust-verified, coverage observable, privacy-opt-in)
...
> **v0.7.5 delta:** Trusted Types `gde-bridge` policy + JSDoc typedefs + `tests/fuzz-extract.test.js` (seeded LCG, 250+ fuzz cases for `canonicalizeUrl`/`extract*`/`fnv1a32`) + 3 ADRs (004–006) + CI `.github/workflows/verify.yml`. **v0.7.4 delta:** `CONFIG.privacy` scrub + `@connect self` + `csp-blocks-bridge` + ADR split 3 →6. **v0.7.3 delta:** `getCoverageMetrics()` frontier + `export.coverage`. See `VERIFICATION_REPORT.md` §2.2 and `CHANGELOG.md`.  
## Candidate Lifecycle
