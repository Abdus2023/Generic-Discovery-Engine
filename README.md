...
> **Current release:** `v0.7.4` — hardening & hygiene (see [`VERIFICATION_REPORT.md`](VERIFICATION_REPORT.md), [`VERIFICATION_SUPPLEMENT_v0.7.2.md`](VERIFICATION_SUPPLEMENT_v0.7.2.md), [`CHANGELOG.md`](CHANGELOG.md), [`dist/generic-discovery-engine.user.js`](dist/generic-discovery-engine.user.js)). `node --check` PASS · `npm test` 42/42 PASS (incl. 6-test E2E + hardening) · `v0.7.1` archived as `dist/generic-discovery-engine.v0.7.1.user.js` · new: [`docs/SECURITY_AUDIT.md`](docs/SECURITY_AUDIT.md), [`docs/PERFORMANCE_ANALYSIS.md`](docs/PERFORMANCE_ANALYSIS.md), [`docs/DECISIONS.md`](docs/DECISIONS.md), [`docs/architecture/OVERVIEW.md`](docs/architecture/OVERVIEW.md), [`docs/adr/`](docs/adr/) · E2E `tests/e2e-discovery-loop.test.js` · `eslint.config.js` + `.prettierrc`.
...
Current response providers (v0.7.4 — 7 providers, ordered pipeline):
...
## Architecture (v0.7.4 — control-plane layered, coverage observable, privacy-opt-in)
...
> **v0.7.4 delta:** `CONFIG.privacy` opt-in scrub + `@connect self` (with `*` commented) + `csp-blocks-bridge` diagnostic + `config-cross-origin-requires-connect-star` warning; ADR split (`docs/DECISIONS.md` + `docs/architecture/OVERVIEW.md` + 3 ADRs) + `eslint.config.js`/`.prettierrc`. **v0.7.3 delta:** `getCoverageMetrics()` + `updateUI` frontier + `exportData.coverage`. **v0.7.2 delta:** `AcquisitionPolicy → AcquisitionPlan → OriginController` + `DecisionLedger` + `ResourceRecord`/`fingerprintIndex`/`graphEdges`. See `VERIFICATION_REPORT.md` §2.2 and `CHANGELOG.md`.  
## Candidate Lifecycle
