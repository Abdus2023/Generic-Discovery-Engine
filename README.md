...
> **Current release:** `v0.7.6` — performance & coverage (see [`VERIFICATION_REPORT.md`](VERIFICATION_REPORT.md), [`VERIFICATION_SUPPLEMENT_v0.7.2.md`](VERIFICATION_SUPPLEMENT_v0.7.2.md), [`VERIFICATION_SUPPLEMENT_v0.7.6.md`](VERIFICATION_SUPPLEMENT_v0.7.6.md), [`CHANGELOG.md`](CHANGELOG.md), [`dist/generic-discovery-engine.user.js`](dist/generic-discovery-engine.user.js)). `node --check` PASS · `npm test` 59/59 PASS (incl. 6-test E2E + hardening + 7 fuzz + 8 property) · `v0.7.1` archived as `dist/generic-discovery-engine.v0.7.1.user.js` · new: [`docs/SECURITY_AUDIT.md`](docs/SECURITY_AUDIT.md), [`docs/PERFORMANCE_ANALYSIS.md`](docs/PERFORMANCE_ANALYSIS.md), [`docs/DECISIONS.md`](docs/DECISIONS.md), [`docs/architecture/OVERVIEW.md`](docs/architecture/OVERVIEW.md), [`docs/adr/`](docs/adr/) 6 ADRs, `tests/fuzz-extract.test.js` + `tests/property-priority.test.js`, `.c8rc.json` + `npm run coverage` (99% line) · `eslint.config.js` + `.prettierrc` + `docs/ci/verify.yml.example`.
...
Current response providers (v0.7.6 — 7 providers, ordered pipeline):
...
## Architecture (v0.7.6 — rAF-batched, coverage observable, privacy-opt-in)
...
> **v0.7.6 delta:** rAF-batched `updateUI` (`_uiRaf` + `_doUpdateUI`) + `npm run coverage` 99% line + `tests/property-priority.test.js` 8 invariants + `VERIFICATION_SUPPLEMENT_v0.7.6.md`. **v0.7.5 delta:** Trusted Types `gde-bridge` + JSDoc + `tests/fuzz-extract.test.js` + 3 ADRs + CI `docs/ci/verify.yml.example`. **v0.7.4 delta:** `CONFIG.privacy` scrub + `@connect self` + `csp-blocks-bridge` + ADR split 3 →6. **v0.7.3 delta:** `getCoverageMetrics()` frontier + `export.coverage`. See `VERIFICATION_REPORT.md` §2.2 and `CHANGELOG.md`.  
## Candidate Lifecycle
